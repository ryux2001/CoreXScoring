...
export default function HomeComponentList({ section, items, currency }: {
  section: HomeSection;
          ...
          return (
            <li key={item.id}>
              ...
              >
                ...
                <span className="min-w-0 flex-1"><strong className="block truncate text-sm text-zinc-200">{item.brand ? `${item.brand} ${item.name}` : item.name}</strong><span className="block truncate text-[10px] font-bold uppercase tracking-wider text-zinc-600">{item.type || 'Componente'}</span></span>
                <span className="shrink-0 text-sm font-black tabular-nums text-white">{formatPrice(price.value, currency)}</span>

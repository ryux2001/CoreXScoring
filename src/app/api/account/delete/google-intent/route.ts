...
export async function POST(request: NextRequest) {
  ...
  let body: { confirmation?: unknown; locale?: unknown } | null;
  try {
    body = await readLimitedJson<{ confirmation?: unknown; locale?: unknown }>(request, 1 * 1024);
  } catch (error) {
  ...
  if (typeof body?.confirmation !== 'string' || typeof body.locale !== 'string' || !isLocale(body.locale) || body.confirmation.trim().toUpperCase() !== getDeleteConfirmationText(body.locale)) {
